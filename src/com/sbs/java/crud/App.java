...
public class App {
	...
	public void start() {
		...
		ArticleController articlecontroller = new ArticleController(sc,memberconteroller);	// 게시물을 관리하는 컨트롤러
		...
		while (true) {
			...
			String controllerName = commandBits[0];		// 잘라서 넣어진 명령어의 첫 단어는 일을 할 컨트롤러의 구분  ex) article
			String actionMethodName = commandBits[1];	// 두번째 단어는 컨트롤러 안의 종류(메소드)의 구분 ex) detail
		
			Controller controller = null;	// 컨트롤러는 초기화를 시켜준다.
		
			if(controllerName.equals("article")) {	// 입력된 명령어의 첫 단어가 제대로 입력되었다면 컨트롤러로 넘긴다.
